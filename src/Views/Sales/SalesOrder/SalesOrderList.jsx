...
import { quotationLists, saleOrderLists } from "../../../Redux/Actions/listApisActions";
...
const SalesOrderList = () => {
  ...
  useEffect(() => {
    const areAllRowsSelected = qutList?.data?.sale_orders?.every((row) => selectedRows.includes(row.id));
    setSelectAll(areAllRowsSelected);
  }, [selectedRows, qutList?.data?.sale_orders]);

  const handleSelectAllChange = () => {
    setSelectAll(!selectAll);
    setSelectedRows(selectAll ? [] : qutList?.data?.sale_orders?.map((row) => row.id));
  };
  //logic for checkBox...
